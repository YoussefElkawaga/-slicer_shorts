...
class AutoPipelineService:
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    logger.info("Redis not available, will use direct async pipeline instead of Celery")
                
                if celery_available:
                    # Submit to Celery (original path)
                    ...
                    # Direct async pipeline execution (no Celery/Redis needed)
                    task.status = TaskStatus.RUNNING
                    task.started_at = datetime.utcnow()
                    db.commit()
                    
                    logger.info(f"启动直接异步流水线处理: {project_id}")
                    
                    # Run pipeline in background
                    asyncio.create_task(
                        self._run_pipeline_direct(project_id, task.id, input_video_path, input_srt_path)
                    )
                    
                    result = {
                        "status": "started",
                        "message": "流水线处理已启动 (直接模式)",
                        "project_id": project_id,
                        "task_id": task.id,
                    }
    ...
    async def _run_pipeline_direct(self, project_id: str, task_id: str, 
                                    input_video_path: str, input_srt_path: str):
        """Run the pipeline directly without Celery/Redis"""
        try:
            logger.info(f"直接异步流水线开始: {project_id}")
            ...
            result = await pipeline.process_project_sync(input_video_path, input_srt_path)
            
            # Update project and task status based on result
            db = SessionLocal()
            try:
                ...
                if result.get("status") == "failed":
                    ...
                    if project:
                        ...
                        project.updated_at = datetime.utcnow()
                    logger.error(f"直接流水线处理失败: {project_id} - {result.get('error')}")
                else:
                    if task:
                    ...
                    if project:
                        ...
                        project.updated_at = datetime.utcnow()
                    logger.info(f"直接流水线处理完成: {project_id}")
            ...
            logger.error(f"直接流水线处理异常: {project_id} - {e}")
            await self._mark_project_failed(project_id, str(e))
        finally:
            self.processing_projects.discard(project_id)
