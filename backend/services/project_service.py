...
class ProjectService(BaseService[Project, ProjectCreate, ProjectUpdate, ProjectResponse]):
    ...
    def get_projects_paginated(
        ...
        """Get paginated projects with filtering and sorting by creation date."""
        ...
        if filters:
            ...
            if getattr(filters, "project_type", None) is not None:
                query = query.filter(Project.project_type == filters.project_type)
            if getattr(filters, "search", None) is not None and filters.search.strip():
        ...
        # Fetch items with descending creation date order
        skip = (pagination.page - 1) * pagination.size
        items = query.order_by(desc(Project.created_at)).offset(skip).limit(pagination.size).all()
