...
@router.get("/shorts-duration-presets")
async def get_shorts_duration_presets() -> Dict[str, Any]:
    """Get available shorts duration presets for the UI."""
    from backend.core.shared_config import SHORTS_DURATION_PRESETS, DEFAULT_SHORTS_DURATION_PRESET
    
    presets = []
    for key, preset in SHORTS_DURATION_PRESETS.items():
        presets.append({
            "value": key,
            "label": preset["label"],
            "description": preset["description"],
            "min_seconds": preset["min_seconds"],
    ...
    return {
        "presets": presets,
        "default_preset": DEFAULT_SHORTS_DURATION_PRESET
    }