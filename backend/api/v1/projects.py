...
async def upload_files(
    ...
    shorts_duration_preset: Optional[str] = Form(None),
    ...
    try:
        ...
        # Save pipeline config (shorts duration preset)
        try:
            import json as json_mod
            from ...core.shared_config import DEFAULT_SHORTS_DURATION_PRESET
            metadata_dir = raw_dir.parent / "metadata"
            metadata_dir.mkdir(parents=True, exist_ok=True)
            pipeline_config = {
                "shorts_duration_preset": shorts_duration_preset or DEFAULT_SHORTS_DURATION_PRESET,
                "video_category": video_category or "knowledge"
            }
            config_path = metadata_dir / "pipeline_config.json"
            with open(config_path, 'w', encoding='utf-8') as f:
                json_mod.dump(pipeline_config, f, ensure_ascii=False, indent=2)
            logger.info(f"Pipeline config saved for upload: {pipeline_config}")
        ...
        try:
            ...
            pipeline_result = await auto_pipeline_service.auto_start_pipeline(project_id)
            logger.info(f"项目 {project_id} 流水线已启动: {pipeline_result}")
        except Exception as e:
