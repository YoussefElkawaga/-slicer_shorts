...
async def create_youtube_download_task(request: YouTubeDownloadRequest):
    ...
    try:
        ...
        try:
            ...
            # Save pipeline config (shorts duration preset) for the pipeline to read
            import json as _json
            metadata_dir = project_dir / "metadata"
            metadata_dir.mkdir(parents=True, exist_ok=True)
            pipeline_config = {
                "shorts_duration_preset": request.shorts_duration_preset or "short",
                "video_category": request.video_category or "default"
            }
            config_path = metadata_dir / "pipeline_config.json"
            with open(config_path, 'w', encoding='utf-8') as cf:
                _json.dump(pipeline_config, cf, ensure_ascii=False, indent=2)
            logger.info(f"Pipeline config saved: {pipeline_config}")
...
async def process_youtube_download_task(task_id: str, request: YouTubeDownloadRequest, project_id: str):
    ...
    try:
        ...
        # Setup download options - skip ALL subtitles to avoid 429 errors
        ydl_opts = {
            'format': 'bestvideo[ext=mp4]+bestaudio[ext=m4a]/bestvideo+bestaudio/best[ext=mp4]/best',
            'merge_output_format': 'mp4',
            'writesubtitles': False,
            'writeautomaticsub': False,
            'subtitleslangs': [],              # Explicitly empty — no subtitle languages
        ...
        def progress_hook(d):
            if d['status'] == 'downloading':
                ...
                if total > 0:
                    pct = 30.0 + (downloaded / total) * 40.0  # Map to 30-70%
                    progress_state['last_pct'] = min(pct, 70.0)
            elif d['status'] == 'finished':
                progress_state['last_pct'] = 70.0
        
        ydl_opts['progress_hooks'] = [progress_hook]
        
        def download_sync(url, opts):
            """Download video, catching subtitle-related errors gracefully."""
            ...
            try:
                ...
                err_msg = str(e)
                # If it's ONLY a subtitle error, the video may still have downloaded
                if 'subtitle' in err_msg.lower() or '429' in err_msg:
                    logger.warning(f"Subtitle download failed (ignored): {err_msg[:120]}")
                else:
                    raise  # Re-raise non-subtitle errors
        
        # Run download with live progress updates
        download_task = asyncio.get_event_loop().run_in_executor(
            None, download_sync, request.url, ydl_opts
        )
        ...
        while not download_task.done():
            ...
            await update_project_download_progress(
                project_id, pct, f"Downloading video... {int(pct)}%"
            )
        ...
        download_tasks[task_id].progress = 75.0
        
        # Update progress
        await update_project_download_progress(project_id, 75.0, "Download complete. Transcribing...")
        
        # If no subtitle file exists, use Groq Whisper API for transcription
        if not subtitle_path:
            logger.info("Using Groq Whisper API for high-quality transcription")
            # Update project progress
            await update_project_download_progress(project_id, 70.0, "Transcribing audio with Groq Whisper...")
            
            try:
                from ...services.groq_transcription import GroqTranscriber, GroqTranscriptionError
                video_file_path = Path(video_path)
                
                transcriber = GroqTranscriber()
                
                if transcriber.is_available():
                    logger.info("Groq API available, starting transcription...")
                    
                    generated_subtitle = transcriber.transcribe_video(
                        video_file_path,
                        language=None  # Auto-detect language (supports Arabic, English, etc.)
                    )
                    subtitle_path = str(generated_subtitle)
                    logger.info(f"Groq transcription complete: {subtitle_path}")
                    
                    # Update project progress
                    await update_project_download_progress(project_id, 90.0, "Transcription complete, preparing...")
                else:
                    logger.warning("Groq API not configured, trying fallback methods...")
                    ...
                    try:
                        ...
                        subtitle_path = str(generated_subtitle)
                        logger.info(f"Fallback Whisper transcription complete: {subtitle_path}")
                        await update_project_download_progress(project_id, 90.0, "Transcription complete, preparing...")
                ...
                # Try platform subtitles as backup
                logger.info("Trying platform subtitle download as backup...")
                try:
                    subtitle_path = await _try_youtube_subtitle_strategies(request.url, download_dir, request.browser)
                    if subtitle_path:
                        logger.info(f"Backup subtitle download succeeded: {subtitle_path}")
                    else:
                        logger.warning("All subtitle strategies failed")
                        subtitle_path = None
                except Exception as backup_error:
                    logger.error(f"Backup subtitle download also failed: {backup_error}")
                    subtitle_path = None
            except Exception as e:
        ...
        try:
            ...
            # Check if subtitle file exists — if not, that's OK
            # Groq Whisper will generate it when processing starts
            srt_file_path = raw_dir / "input.srt"
            if not srt_file_path.exists():
                logger.info(f"No subtitle file yet for project {project_id} — Groq Whisper will generate it later")
        ...
        logger.error(f"Download task failed: {error_msg}")
        download_tasks[task_id].status = "failed"
        download_tasks[task_id].error_message = error_msg
        download_tasks[task_id].progress = 0.0
        download_tasks[task_id].updated_at = datetime.now().isoformat()
        
        # Update project status in DB so frontend sees the failure
        try:
            await update_project_download_progress(project_id, 0.0, f"Failed: {error_msg[:100]}")
            from ...core.database import SessionLocal
            from ...services.project_service import ProjectService
            from ...schemas.project import ProjectStatus
            db2 = SessionLocal()
            try:
                ps = ProjectService(db2)
